...
api.login = function (userId, userName, headurl, cb) {
  api.timaccounts({
    ...
    webim.login(loginInfo, listeners, options, resp => cb && cb(null, loginInfo), err => cb && cb(err.ErrorInfo)
    );
  }).catch(err => cb && cb(err));
};
...
api.getRecentContactList = function (cb) {
  webim.getRecentContactList({
    ...
    const sessionItem = resp.SessionItem || [];
    const ids = [];
    const idMap = {};

    for (let i = 0, len = sessionItem.length; i < len; i++) {
      console.log('sessionitem -->', sessionItem[i]);
      ids.push(sessionItem[i].ToAccount);
      idMap[sessionItem[i].ToAccount] = { txSession: sessionItem[i] };
    }
    ...
    imAPI.getDialogList({
      ...
      for (let i = 0, len = rs.data.length; i < len; i++) {
        idMap[rs.data[i]._id].dialogInfo = rs.data[i];
        newDoc.push(idMap[rs.data[i]._id]);
      }

      return cb && cb(null, newDoc);
    }).catch(err => cb && cb(err));
    // 业务处理
  }, resp =>
    // 错误回调
...
const getGroupHistoryMessage = function getGroupHistoryMessage(toId, cb) {
  ...
  api.getGroupInfo(toId, (err, resp) => {
    if (err) {
      return cb && cb(err);
    }

    console.log('resp.GroupInfo --->', resp.GroupInfo);
...
// 邀请好友加群
api.addGroupMember = function (groupId, userId, cb) {
  const options = { GroupId: groupId,
    MemberList: [{
      Member_Account: userId
    }]
  };
  webim.addGroupMember(options, resp => cb && cb(null, '邀请好友加群成功'), err => cb && cb(err.ErrorInfo));
};
api.on = bubble.on;
api.off = bubble.off;
