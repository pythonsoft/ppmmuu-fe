<template>
  <div>
    <four-row-layout-right-content>
      <template slot="search-left">{{title}}</template>
      <template slot="search-right">
        <div class="audit-download-search-item" :style="{ width: '100px' }" v-if="isShowSelect">
  ...
  export default {
    ...
    methods: {
      updateStatus() {
        ...
        for (let i = 0; i < keys.length; i++) {
          ...
          if (this.$route.name.indexOf(key.toLowerCase()) > -1) {
            ...
            this.status = this.options[key].value;
            this.title = this.options[key].text;
            this.page = 1;
            this.listAudit();
            break;
          }
      ...
      listAudit() {
        ...
        api.listAuditJob(formatQuery(searchObj, true), me).then((res) => {
          const data = res.data;
